...
//    assert(None.asInstanceOf[Any] === AnyRef)
//  assert(None.asInstanceOf[AnyRef] === AnyRef)
//  assert(None.asInstanceOf[AnyVal] === AnyRef)
//  }

  koan("None cannot be cast to all types of objects") {
    intercept[ClassCastException] {
      // put the exception you expect to see in place of the blank
      assert(None.asInstanceOf[String] === "Something")
    }
  }
